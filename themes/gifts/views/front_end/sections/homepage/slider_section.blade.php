<section class="home-banner-section padding-top padding-bottom" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
    <img src="{{ asset('themes/' . $currentTheme . '/assets/img/home-banner-ptrn.png') }}" class="home-banner-bg" alt="">
    ...
    <div class="offset-container offset-left">
                            ...
                            <div class="col-md-6 col-12">
                                <div class="banner-left-col">
                                    <div class="section-title">
                                        <div class="subtitle" id="{{ ($section->slider->section->title->slug ?? '').'_'. $i }}_preview">{!!
                            ...
                            ($section->slider->section->description->text->{$i} ?? '') !!}</p>
                                    <div class="button-wrappper d-flex">
                                        <a href="{{ route('page.product-list', $slug) }}" class="btn"
                                            tabindex="0" id="{{ ($section->slider->section->button->slug ?? '').'_'. $i }}_preview">{!!
                            ...
                            <div class="col-md-6 col-12">
                                <div class="banner-right-col">
                                    <div class="hero-main-img">
                                        <img src="{{ get_file($section->slider->section->image->image->{$i} ?? '', $currentTheme) }}"
                                            class="img-fluid {{ ($section->slider->section->image->slug ?? '').'_'.$i}}_preview" alt="HeroImage-1">
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="slider-navgation">
            ...
            <div class="progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="50">
                <span class="slider__label sr-only">{{ __('50% completed') }}</span>
            </div>
        </div>
    </div>
</section>
