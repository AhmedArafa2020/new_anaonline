<section class="offer-bestseller-section padding-bottom" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
    <div class="container">
        <div class="section-title d-flex align-items-center justify-content-between">
            <div class="section-title-left">
                <h2 id="{{ $section->best_product->section->title->slug ?? '' }}_preview">{!! $section->best_product->section->title->text ?? '' !!}
                </h2>
            </div>
            <a href="{{ route('page.product-list', $slug) }}" class="btn" tabindex="0" id="{{ $section->best_product->section->button->slug ?? '' }}_preview">{!! $section->best_product->section->button->text ?? '' !!}
            </a>
        </div>
        <div class="row">
            @foreach ($all_products->take(2) as $homepage_product)
                <div class="col-md-6 col-12 offer-bestseller-card">
                    <div class="bestseller-card-itm ">
                        <div class="bestseller-card-inner product-card">
                            <div class="bestseller-content">
                                <div class="bestseller-top">
                                
                                    <div class="bestseller-card-heading">
                                        <span>{{!empty($homepage_product->ProductData) ? $homepage_product->ProductData->name : ''}}</span>
                                    ...
                                    <p class="descriptions">{!! strip_tags($homepage_product->description) !!}</p>
                                </div>
                                <div class="bestseller-bottom">
                                    <div class="bestseller-price-wrapper">
                                    @if ($homepage_product->variant_product == 0)
                                        <div class="price">
                                        {!! \App\Models\Product::getProductPrice($homepage_product, $store, $currentTheme) !!}
                                    ...
                                    {!! \App\Models\Product::ProductcardButton($currentTheme, $slug, $homepage_product) !!}
                                </div>
                            </div>
                            <div class="bestseller-img">
                                <a href="{{ url($slug.'/product/'.$homepage_product->slug) }}">
                                    <img src="{{ get_file($homepage_product->cover_image_path, $currentTheme) }}"
                                        alt="">
                                </a>
                            </div>

                            {!! \App\Models\Product::actionLinks($currentTheme, $slug, $homepage_product) !!}
                        </div>
                    </div>
                </div>
            @endforeach
