        ...
        <div class="row">
            <div class="col-lg-6 col-12 add-banner-card">
                <div class="add-banner">
                    <img src="{{ get_file($section->product_category->section->image->image ?? '', $currentTheme) }}" class="add-bnr">
                    <div class="add-banner-column">
                        <div class="section-title">
                            <h3 id="{{ ($section->product_category->section->title->slug ?? '') }}_preview">{!!
                                $section->product_category->section->title->text ?? "" !!}</h3>
                        </div>
                        ...
                        <div class="add-banner-section-btn">
                            <a href="#" class="btn-secondary" tabindex="0" id="{{ ($section->product_category->section->button->slug ?? '') }}_preview">{!!
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-12 add-banner-card">
                <div class="add-banner">
                    <img src="{{ get_file($category->image_path ?? '', $currentTheme) }}" alt="" class="add-bnr">
                    <div class="add-banner-column">
                        <div class="section-title">
                            <h3> {!! $category->name !!} </h3>
                        </div>
                        <div class="add-banner-section-btn">
                            <a href="{{ route('page.product-list', [$slug, 'main_category' => $category->id]) }}" class="btn-secondary" tabindex="0">
                                {!! $section->product_category->section->button->text ?? "" !!}
                                <svg xmlns="http://www.w3.org/2000/svg" class="ms-2" width="14" height="16"
                                    viewBox="0 0 14 16" fill="none">
                                    <path fill-rule="evenodd" clip-rule="evenodd"
