<section class="pdp-page review-section padding-bottom" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
        ...
        <div class="row align-items-center col-reverse">
            <div class="col-md-6 col-12">
                <div class="left-review-wrapper">
                    <div class="section-title">
                        <h2 id="{{ $section->best_product_second->section->title->slug ?? '' }}_preview">{!! $section->best_product_second->section->title->text ?? '' !!}</h2>

                    </div>
                    <p id="{{ $section->best_product_second->section->title->slug ?? '' }}_preview">{!! $section->best_product_second->section->title->text ?? '' !!}</p>
                        ...
                        <div class="review-itm">
                            <div class="review-inner">
                                ...
                                <div class="review-content">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="cauma" width="13" height="10"
            ...
            <div class="col-md-6 col-12">
                <div class="right-review-wrapper">
                    <img src="{{ get_file( $section->best_product_second->section->image->image ?? '') }}" alt="" id="{{ $section->best_product_second->section->image->slug ?? '' }}_preview">
                </div>
            </div>
        </div>
    </div>
