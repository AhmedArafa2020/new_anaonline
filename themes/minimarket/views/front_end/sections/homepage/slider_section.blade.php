<section class="hero-sec"
    ...
    <img src="{{ get_file($section->slider->section->background_image->image ?? '', $currentTheme)}}" id="{{ ($section->slider->section->background_image->slug ?? '').'_preview'}}" alt="banner-img" loading="lazy" class="sixth-sec-banner">
    <div class=" container">
                ...
                <div class="hero-slides common-heading">
                    ...
                    <div class="hero-btn">
                        <a href="{{ route('page.product-list', $slug) }}" class="common-btn">
                            <span id="{{ ($section->slider->section->button_first->slug ?? '').'_'. $i }}_preview">{!!
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="45" height="44" viewBox="0 0 45 44" fill="none">
                            ...
                            <span id="{{ ($section->slider->section->button_second->slug ?? '').'_'. $i }}_preview">{!!
                                $section->slider->section->button_second->text->{$i} ?? '' !!}</span>
                        </a>
                    </div>
                </div>
                @endfor
