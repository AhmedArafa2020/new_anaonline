<section class="card-sec padding-bottom"
    ...
    <img src="{{ asset('themes/' .$currentTheme . '/assets/images/d4.png') }} " class="d-left" style="top: 25%;">
    <img src="{{ asset('themes/' .$currentTheme . '/assets/images/right.png') }}" class="d-right" style="bottom: 0">
    <img src="{{ asset('themes/' .$currentTheme . '/assets/images/leaf2.png') }}" class="desk-only" id="card-leaf2"
        alt="leaf2">
    <div class=" container">
        <div class="row">
            <div class="col-lg-6 col-12 img-main-box">
                <div class="img-box">
                    <img src="{{ get_file($section->best_product->section->image->image ?? '', $currentTheme) }}" class="card-img"
                        id="{{ ($section->best_product->section->image->slug ?? '').'_preview'}}" alt="img1">
                    <div class="inner-text">
                        <div class="common-heading">
                            <span class="sub-heading"
                                id="{{ ($section->best_product->section->sub_title->slug ?? '') }}_preview">
                                {!!$section->best_product->section->sub_title->text ?? "" !!}</span>
                            <h3 id="{{ ($section->best_product->section->title->slug ?? '') }}_preview">
                                <a href="{{ route('page.product-list', $slug) }}">{!!
                                    $section->best_product->section->title->text ?? ""!!}</b></a>
                            </h3>
                            ...
                            <div class=" d-flex justify-content-center align-items-center">
                                <a href="{{ route('page.product-list', $slug) }}" class="common-btn"
                                    id="{{ ($section->best_product->section->button->slug ?? '') }}_preview">{!!$section->best_product->section->button->text
                                    !!}</a>
                            </div>
                        </div>
                    </div>
