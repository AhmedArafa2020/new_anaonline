...
<div class="col-lg-3 col-md-4 col-sm-6 col-12 blog-card">
            <div class="article-card">
                ...
                <div class="card-content blog-caption">
                    ...
                    <span class="date"> <a href="#">{{ $blog->store->user->name ?? 'John Doe' }}</a> • {{$blog->created_at->format('d M,Y ')}}</span>
                    <a href="{{route('page.article',[$slug,$blog->id])}}" class="common-btn2">{{ $page_json->blog_page->section->section_sub_button->text ??__('Read More') }}</a>
                </div>
            </div>
        </div>
    @endif
