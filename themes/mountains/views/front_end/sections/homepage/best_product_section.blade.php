<section class="place-section padding-bottom" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}" data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
    <div class="row no-gutters justify-content-between align-items-center flex-dairection">
        <div class="col-lg-4 col-md-4 col-12">
            <div class="place-left">
                <img src="{{ get_file($section->best_product->section->image->image ?? '', $currentTheme)}}" class="place-left-one" alt="">
                <img src="{{ get_file($section->best_product->section->background_image->image ?? '', $currentTheme)}}" class="place-left-two" alt="">
            </div>
        </div>
        <div class="col-lg-4 col-md-4 col-12">
            <div class="place-section-center">
                <div class="section-title">
                    <span class="subtitle" id="{{ $section->best_product->section->sub_title->slug ?? '' }}_preview">
                        {!! $section->best_product->section->sub_title->text ?? '' !!}</span>
                    <h2 id="{{ $section->best_product->section->title->slug ?? '' }}_preview">
                        {!! $section->best_product->section->title->text ?? '' !!}
                    </h2>
                </div>
        ...
        <div class="col-lg-4 col-md-4 col-12">
            <div class="place-right">
                <div class="place-right-image">
                    <img class="" alt="Place-right-img" src="{{ get_file($section->best_product->section->service_third_image->image ?? '', $currentTheme)}}" class="place-left-one" alt="">
                </div>
                <div class="place-right-image">
                    <img alt="Place-right-img" src="{{ get_file($section->best_product->section->service_second_image->image ?? '', $currentTheme)}}" class="place-left-one" alt="">
                </div>
                <div class="place-right-image">
                    <img alt="Place-right-img" src="{{ get_file($section->best_product->section->service_image->image ?? '', $currentTheme)}}" class="place-left-one" alt="">
                </div>
            </div>
        </div>
    </div>
