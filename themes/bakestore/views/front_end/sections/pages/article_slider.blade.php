<div class="container">
    <div class="d-flex justify-content-between align-items-center ">
        <div class="section-title">
            ...
            <span
                class="sub-title">{{ $page_json->article_page->section->section_title->text ?? __('All Products') }}</span>
            <h3>{!!$page_json->article_page->section->section_sub_title->text ?? __('From  our blog') !!}</h3>
        </div>
        <a href="#" class="btn">{{ $page_json->article_page->section->section_sub_button->text ?? __('Read More') }}</a>
    </div>
    <div class="blog-head-row d-flex justify-content-between">
        <div class="blog-col-left">
            <ul class="d-flex tabs">
                @foreach ($BlogCategory as $cat_key => $category)
                    <li class="tab-link on-tab-click {{ $cat_key == 0 ? 'active' : '' }}" data-tab="{{ $cat_key }}"><a
                                ...
                                <div class="blog-card ">
                                    <div class="blog-card-inner">
                                        ...
                                        <div class="blog-card-content">
                                            ...
                                            <div class="section-title">
                                                <h3>
                                                    <a href="{{route('page.article', ['storeSlug' => $slug, $blog->id])}}" class="name">
                                                        {{$blog->title}}
                                                        {{$blog->category->name}}
                                                    </a>
                                                </h3>
                                            ...
                                            <div class="blog-card-bottom">
                                                <a href="{{route('page.article', ['storeSlug' => $slug, $blog->id])}}" class=" btn">
