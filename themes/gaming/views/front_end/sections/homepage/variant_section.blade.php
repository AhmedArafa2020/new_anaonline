<section class="full-width-section padding-top padding-bottom" tyle="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif"
    data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}"
    data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"
    data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
    <img src="{{asset('themes/'.$currentTheme.'/assets/images/gaming-products.png')}}" class="gaming-products-1" alt="image">
    <img src="{{asset('themes/'.$currentTheme.'/assets/images/gaming-products-1.png')}}" class="gaming-products-2" alt="image">
    <img src="{{asset('themes/'.$currentTheme.'/assets/images/design-circle-4.png')}}" class="design-circle-4" alt="image">
    <img src="{{asset('themes/'.$currentTheme.'/assets/images/design-circle-5.png')}}" class="design-circle-5" alt="image">
    <div class="container">
        <div class="section-title d-flex justify-content-between align-items-center">
            ...
            <div class="section-title-right">
                <a href="{{route('page.product-list',$slug)}}" class="btn-primary btn-transparent" id="{{ $section->variant_background->section->button->slug ?? '' }}_preview">
        ...
        <div class="d-flex align-items-center">
            <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6  col-12">
                <div class="left-column-img">
                    <img  src="{{ get_file($section->variant_background->section->image_left->image ?? '', $currentTheme) }}"
                        class="cpu-left {{ ($section->variant_background->section->image_left->slug ?? '').'_preview'}}" alt="image" >
                </div>
            </div>
            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 col-12">
                @if(!empty($latest_product))
                    <div class="full-width-card product-card">
            ...
            <div class="col-xl-5 col-lg-4 col-md-6 col-12">
                <div class="right-column-img">
                    <img src="{{ get_file($section->variant_background->section->image_right->image ?? '', $currentTheme) }}"
                    class="cpu-right {{ ($section->variant_background->section->image_right->slug ?? '').'_preview'}}" alt="image">
                </div>
            </div>
        </div>
    </div>
