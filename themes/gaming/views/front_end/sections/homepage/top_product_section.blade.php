<section class="bestseller-section padding-bottom"
    ...
    <img src="{{asset('themes/'.$currentTheme.'/assets/images/design-circle-3.png')}}" class="design-circle-3"
        alt="image">
    <div class="container">
        <div class="section-title d-flex align-items-center justify-content-between">
            ...
            <a href="{{route('page.product-list',$slug)}}" class="btn-transparent"
                id="{{ $section->top_product->section->button->slug ?? '' }}_preview"> {!!
                $section->top_product->section->button->text ?? '' !!}
    ...
    <div class="product-center-slider">
        ...
        <div class="product-center-item product-card">
            <div class="product-card-inner">
                <div class="card-top">
                    <span
                        class="slide-label">{{!empty($bestSellers->ProductData) ? $bestSellers->ProductData->name : ''}}
                    </span>
                ...
                <div class="product-card-image">
                    <a href="{{url($slug.'/product/'.$bestSellers->slug)}}">
                        <img src="{{ get_file($bestSellers->cover_image_path ?? '', $currentTheme) }}" class="default-img">
                        @if($bestSellers->Sub_image($bestSellers->id)['status'] == true)
                        <img src="{{ get_file($bestSellers->Sub_image($bestSellers->id)['data'][0]->image_path, $currentTheme) }}"
                            class="hover-img">
                        @else
                        <img src="{{ get_file($bestSellers->Sub_image($bestSellers->id), $currentTheme) }}" class="hover-img">
                        @endif
                    </a>
                </div>
