<section
    class="travel-info-section"style="position: relative;@if (isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif"
        ...
        <div class="row">
            <div class="col-md-6 col-lg-6 col-xl-5 col-12">
                <div class="travel-image-left">
                    <img src="{{ get_file($section->category->section->background_image->image ?? '', $currentTheme) }}" alt="">
                </div>
            </div>
            <div class="col-md-6 col-lg-6 col-xl-4 col-12">
                <div class="travel-info-right">
                    <div class="offer-announcement second-style">
                        <span class="new-labl"
                            id="{{ $section->category->section->button->slug ?? '' }}_preview">{!! $section->category->section->button->text ?? '' !!}</span>
                        <p id="{{ $section->category->section->sub_title->slug ?? '' }}_preview">
                            <b>{!! $section->category->section->sub_title->text ?? '' !!} <a href="#">More</a></b></p>
                    </div>
                    <div class="section-title">
                        <h2 id="{{ $section->category->section->title->slug ?? '' }}_preview">{!! $section->category->section->title->text ?? '' !!}
                        </h2>
                    </div>
                    <p id="{{ $section->category->section->description->slug ?? '' }}_preview">{!! $section->category->section->description->text ?? '' !!}
                    </p>
                    <a href="{{ route('page.product-list', $slug) }}" class="btn-secondary"
                        id="{{ $section->category->section->button_second->slug ?? '' }}_preview"><svg
                            ...
                            <path
                                ...
                                fill="white"></path>
                        </svg>{!! $section->category->section->button_second->text ?? '' !!}</a>
                </div>
            </div>
        </div>
    </div>
