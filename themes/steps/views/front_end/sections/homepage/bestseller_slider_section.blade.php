<section class="bestseller-section padding-top padding-bottom" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
        <img src="{{ asset('themes/' . $currentTheme . '/assets/images/rightt.png') }}" class="right-shape">
        <div class="container">
            <div class="section-title d-flex align-items-center justify-content-between">

                    <div class="section-title-left">
                        <h2 class="xl-text" id="{{ $section->bestseller_slider->section->title->slug ?? ''}}_preview">
                        {!! $section->bestseller_slider->section->title->text ?? ''!!}</h2>

                    </div>
                    <a href="{{ route('page.product-list',  ['storeSlug' => $slug]) }}" class="btn-secondary white-btn">
                        <span class="btn-txt" id="{{ $section->bestseller_slider->section->button->slug ?? ''}}_preview">
                {!! $section->bestseller_slider->section->button->text ?? '' !!}</span>
                        <span class="btn-ic">
                    ...
                    <div class="bestsell-slider common-arrow  dark-bg">
                        @foreach ($bestSeller as $data)
                            @if ($cat_k == '0' || $data->ProductData->id == $cat_k)
                                <div class="bestsell-itm product-card">
                                        ...
                                        <div class="bestsell-content">
                                            <div class="bestsell-content-row">
                                                <div class="bestsell-content-left">
                                                        <h3><a href="{{ url($slug.'/product/'.$data->slug)}}">{{ $data->name }}</a>
                                                    </h3>
                                                    <h4><a href="{{ url($slug.'/product/'.$data->slug)}}">{{ $data->ProductData->name }}</a>
                                                    </h4>
                                                ...
                                                <div class="bestsell-content-right">
