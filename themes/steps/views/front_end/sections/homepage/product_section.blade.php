<section class="all-the-shoes" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
<div class="custome_tool_bar"></div>
<img src="{{ get_file($section->product->section->image->image, $currentTheme) }}" id="{{ $section->product->section->image->slug ?? '' }}_preview" alt="" class="left-shape">
        <div class="container">
            <div class="section-title d-flex align-items-center justify-content-between">

                    <div class="section-title-left">
                        <div class="subtitle" id="{{ $section->product->section->sub_title->slug ?? ''}}_preview">
                {!! $section->product->section->sub_title->text ?? '' !!}</div>
                ...
                {!! $section->product->section->title->text ?? '' !!}</h2>
                    ...
                    <a href="{{ route('page.product-list', $slug) }}" class="btn-secondary">
                        <span class="btn-txt" id="{{ $section->product->section->button->slug ?? ''}}_preview">
                {!! $section->product->section->button->text ?? '' !!}</span>
                        <span class="btn-ic">
            ...
            <div class="all-shoes-slider flex-slider">
                @foreach ($all_products as $product)
                    ...
                    <div class="all-shoes-slide-itm card product-card">
                        <div class="all-shoes-itm-inner card-inner d-flex ">
                            ...
                            <div class="all-shoes-itm-img">
                                ...
                                <a href="{{ url($slug.'/product/'.$product->slug) }}" class="all-shoe-img">
