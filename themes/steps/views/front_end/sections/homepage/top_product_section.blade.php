<section class="shoe-two-column-layput two-col-bottom" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif"
            ...
            <div class="row no-gutters">
                <div class="col-md-5 col-12 d-flex align-items-center">
                    <div class="two-coll-content">
                        @if (!empty($landing_product))
                            ...
                            <div class="lable-btn btn" id="{{ $section->top_product->section->title->slug ?? ''}}_preview">{!! $section->top_product->section->title->text ?? ''!!}</div>
                                <div class="thmb-pro-main">
                                    <div class="thumb-pro-main-itm">
                                        <a href="{{ url($slug.'/product/'.$landing_product->slug) }}">
                                            <img
                                                src=" {{ get_file($landing_product->cover_image_path ?? '', $currentTheme) }}"></a>
                                    </div>
                ...
                <div class="col-md-7 col-12 d-flex">
                    <div class="two-coll-media">
                        <div class="two-coll-media">
                            <img src="{{ get_file($section->top_product->section->image->image, $currentTheme) }}" id="{{ $section->top_product->section->image->slug ?? '' }}_preview">
                        </div>
                    </div>
                </div>
            </div>
