<section class="modern-category-section padding-bottom padding-top" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    ...
    <div class="container">
        <div class="section-title d-flex align-items-center justify-content-between">
            ...
            <a href="{{route('page.product-list',$slug)}}" class="btn-secondary" id="{{ ($section->category->section->button->slug ?? '') }}_preview">{!! $section->category->section->button->text ?? '' !!}
        ...
        <div class="row no-gutters">
            @foreach ($MainCategoryList->take(4) as $category)
                    ...
                    <a href="{{route('page.product-list',[$slug,'main_category' => $category->id ])}}" class=" row align-items-center">
                        ...
                        <div class="modern-cat-conent col-md-9 col-sm-8 col-8">
                            ...
                            <div class="go-to-btn">
                                ...
                                <span>[{{$category->product_details()->count() ?? 0}}]</span>
                            </div>
                        </div>
                    </a>
                </div>
