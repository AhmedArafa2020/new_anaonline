@php
    ...
    $BlogCategory = \App\Models\BlogCategory::where('theme_id', $store->theme_id)->where('store_id',$store->id)->get()->pluck('name','id');
    $BlogCategory->prepend('All Products','0');
@endphp
<div class="container">
    <div class="blog-grid-title d-flex justify-content-between align-items-center ">
        <div class="section-title">
            <div class="sub-title">{{ $page_json->article_page->section->section_title->text ?? __('Blog & Articles') }}</div>
            <h2>{!!$page_json->article_page->section->section_sub_title->text ?? __('From our blog') !!}</h2>
        </div>
