<section class="third-sec mb-6" style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
    <div class=" container">
        <div class="third-sec-row row align-items-left justify-content-between">
            <div class=" col-md-7">
            ...
            <div class=" col-md-5">
                <div class=" third-sec-img-div">
                    <img src="{{ get_file($section->category->section->background_image->image, $currentTheme) }}" alt="img1">
                </div>
            </div>
        </div>
    </div>
