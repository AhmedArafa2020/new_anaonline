<section class="card-sec mb-6"style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
    <img src="{{ asset('themes/' .$currentTheme . '/assets/images/leaf2.png') }}" class="desk-only"
        id="card-leaf2" alt="leaf2">
        <div class=" container">
        <div class="row">
            <div class="col-lg-6 col-12 img-main-box">
                <div class="img-box">
                        <img src="{{ get_file($section->best_product->section->image->image, $currentTheme) }}"class="card-img"
                        alt="img1">
                    <div class="inner-text">
                        <div class="common-heading">
                            <span class="sub-heading" id="{{ ($section->best_product->section->sub_title->slug ?? '') }}_preview"> {!!$section->best_product->section->sub_title->text ?? "" !!}</span>
                            <h5  id="{{ ($section->best_product->section->title->slug ?? '') }}_preview"><a
                                    href="{{ route('page.product-list', $slug) }}">{!! $section->best_product->section->title->text ?? ""!!}</b></a>
                            </h5>
                            ...
                            <div class=" d-flex justify-content-left align-items-center">
                                <a href="{{ route('page.product-list', $slug) }}"
                                    class="common-btn" id="{{ ($section->best_product->section->button->slug ?? '') }}_preview">{!!$section->best_product->section->button->text !!}</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="main-card product-card card col-lg-3 col-sm-6 col-12">
                    ...
                    <div class="inner-card">
                        <div class="wishlist-wrapper">
                            <a href="JavaScript:void(0)"
                                class="add-wishlist wishlist wishbtn wishbtn-globaly" title="Wishlist" tabindex="0"
                                product_id="{{ $data->id }}" in_wishlist="{{ $data->in_whishlist ? 'remove' : 'add' }}">
                                <span class="wish-ic">
