<section class="filter-sec mb-6"style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    ...
    <div class=" container">
        <div class="title d-flex justify-content-between align-items-center flex-wrap">
            ...
            <ul class="category-buttons d-flex tabs">
                ...
                <li class="tab-link {{ $cat_key == 0 ? 'active' : '' }}" data-tab="{{ $cat_key }}_data"><a
                            ...
                            @if ($cat_k == '0' || $homeproduct->ProductData->id == $cat_k)
                                <div class="main-card card product-card">
                                    <div class="pro-card-inner">
                                        <a href="{{ route('page.product',['storeSlug' => $slug, 'product_slug' => $homeproduct->slug]) }}"
                                            class="img-wrapper">
                                            <img src="{{ get_file($homeproduct->cover_image_path ?? '', $currentTheme) }}"
