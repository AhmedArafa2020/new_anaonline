...
<section class="top-bg-wrapper"
 style="background-image:url({{ get_file( $page_json->article_page->section->image->image ?? 'themes/' . $currentTheme . '/assets/images/article-pic.jpg', $currentTheme)}});">
 <div class=" container">
             ...
             <div class="col-12">
                <div class="about-user d-flex align-items-center">
                   <div class="abt-user-img">
                      <img src="{{ asset( $page_json->article_page->section->image->image ??'themes/' . $currentTheme. '/assets/images/article-user.png')}}">
                   </div>
                      ...
                      <div class="col-md-12 col-sm-6 col-12 blog-widget">
                        <div class="article-card blog-caption">
                            ...
                            <div class="card-content">
                               ...
                               <h3><a href="{{route('page.article',[$slug,$data->id])}}">{{$data->title}}</a></h3>
                               <p>{{$data->short_description}}</p>
                               <span class="date"> <a href="{{route('page.article',[$slug,$data->id])}}">{{ $blog->store->user->name ?? '@John' }}</a> • {{$blog->created_at->format('d M, Y ')}}</span>
                               <a href="{{route('page.article',[$slug,$data->id])}}" class="common-btn2">{{ $page_json->article_page->section->related_button->text ?? __('Read More') }} </a>
                            </div>
                         </div>
                      </div>
                      @endforeach
                ...
                @if($cat_k == '0' ||  $blog->category_id == $cat_k)
                    ...
                    <div class="col-lg-3 col-md-4 col-sm-6 col-12 blog-card">
                        ...
                        <div class="card-content">
                            ...
                            <a href="{{route('page.article',[$slug,$blog->id])}}" class="common-btn2">{{ $page_json->article_page->section->latest_button->text ?? __('Read More') }}</a>
                        </div>
                        </div>
                    </div>
                @endif
