            ...
            <div class="col-lg-6 col-md-4 col-12 d-flex">
                <div class="columnl-left-media-inner">
                    <div class="decorative-text"
                        ...
                        {!! $section->modern_product->section->sub_title->text ?? '' !!}
                    </div>
                    <img src="{{ get_file($section->modern_product->section->image->image, $currentTheme) }}" alt="room">
                    <div class="column-left-media-content">
                        <div class="section-title">
                            <h3 id="{{ $section->modern_product->section->title->slug ?? '' }}_preview">
                                {!! $section->modern_product->section->title->text ?? '' !!}</h3>
                        </div>
                ...
                <div class="row h-100">
                    @foreach ($home_products->take(2) as $h_product)
                        <div class="col-lg-6 col-md-6 col-sm-6 col-12 d-flex">
                            <div class="columnl-right-caption-inner product-card w-100">
                                <div class="product-card-inner">
                                    <div class="product-card-image">
                                        <a href="{{ url($slug.'/product/'.$h_product->slug) }}"
                                            class="product-img">
                                            <img src="{{ get_file($h_product->cover_image_path ?? '', $currentTheme) }}" class="default-img">
                                        </a>
                                        <div class="new-labl">
                                            <a href="javascript:void(0)" class="wishbtn wishbtn-globaly"
                                                product_id="{{ $h_product->slug }}"
                                                in_wishlist="{{ $h_product->in_whishlist ? 'remove' : 'add' }}">
                                                <span class="wish-ic">
                                                    <i class="{{ $h_product->in_whishlist ? 'fa fa-heart' : 'ti ti-heart' }}"
                                                       ></i>
                                                </span>
                                            </a>

                                        </div>
                                        {!! \App\Models\Product::actionLinks($currentTheme, $slug, $h_product) !!}
                                    </div>
                                        ...
                                        <div class="product-content-top">
                                            <div class="review-star">
                                                ...
                                                <div class="d-flex align-items-center">
                                                    @for ($i = 0; $i < 5; $i++)
                                                        <i
                                                            class="fa fa-star review-stars {{ $i < $h_product->average_rating ? 'text-warning' : '' }} "></i>
                                                    @endfor
                                                </div>
                                            </div>
                                            <h3 class="product-title"><a class="title"
                                                    href="{{ url($slug.'/product/'.$h_product->slug) }}">
                                                    {{ $h_product->name }}</a></h3>
                                        </div>
                                        <div class="product-content-bottom">
                                            <div class="pro-bottom-price">
                                                @if ($h_product->variant_product == 0)
                                                    <div class="price">
                                                    {!! \App\Models\Product::getProductPrice($h_product, $store, $currentTheme) !!}
                                                    </div>
                                                @else
                                                    <div class="price">
                                                ...
                                                {!! \App\Models\Product::productSalesPage($currentTheme, $slug, $h_product->id) !!}

                                            </div>
                                            <a href="javascript:void(0)" class="btn-secondary addcart-btn-globaly"
                                                product_id="{{ $h_product->slug }}" variant_id="0" qty="1">
                                            ...
                                            {!! \App\Models\Product::ProductcardButton($currentTheme, $slug, $h_product) !!}
                                        </div>
                                    </div>
                                </div>
                            </div>
