...
style="background-image:url({{ get_file( $page_json->blog_page->section->image->image ?? 'themes/' . $currentTheme . '/assets/images/article-pic.jpg', $currentTheme)}});">
<div class=" container">
              <div class="col-md-6 col-12">
                 <div class="common-banner-content">
                    ...
                    <a href="{{route('landing_page',$slug) }}" class="common-btn2 white-btn" tabindex="0">
                        {{ $page_json->blog_page->section->button->text ?? __('BACK TO CATEGORY') }}
                    </a>
                 </div>
              </div>
           </div>
            ...
            <div class="container">
              <div class="section-title d-flex justify-content-between align-items-end padding-top">
                 <div class="blog-title">
                    <span>{{ $page_json->blog_page->section->section_title->text ?? __('All PRODUCTS') }}</span>
                                    ...
                                    <div class="col-lg-3 col-md-4 col-sm-6 col-12 blog-card">
                                        <div class="article-card">
                                            ...
                                            <div class="card-content blog-caption">
                                                ...
                                                <span class="date"> <a href="{{route('page.article',[$slug,$blog->id])}}">{{ $blog->store->user->name ?? '@john' }}</a> • {{$blog->created_at->format('d M,Y ')}}</span>
                                                <a href="{{route('page.article',[$slug,$blog->id])}}" class="common-btn2">{{__('Read More')}}</a>
                                            </div>
                                        </div>
                                    </div>
                                @endif
