<section class="testimonials-sec mb-6"style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
    <div class="container">
        <div class="common-heading">
            <span class="sub-heading" id="{{ $section->review->section->title->slug ?? '' }}_preview"> {!!  $section->review->section->title->text ?? '' !!}</span>
            <h2 id="{{ $section->review->section->description->slug ?? '' }}_preview">{!!  $section->review->section->description->text ?? '' !!}</h2>
        </div>
                            ...
                            <div class="card-inner">
                                ...
                                <div class="inner-card">
                                    <div class="wishlist-wrapper">
                                        ...
                                        {!! \App\Models\Product::actionLinks($currentTheme, $slug, $review->ProductData) !!}
                                        {!! \App\Models\Product::productSalesPage($currentTheme, $slug, $review->ProductData->id) !!}
                                    </div>
                                    ...
                                    {!! \App\Models\Product::ProductcardButton($currentTheme, $slug, $review->ProductData) !!}
                                </div>
                            </div>
                        </div>
                    @endforeach
