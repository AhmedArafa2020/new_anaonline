<section class="fifth-sec"style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    <div class="custome_tool_bar"></div>
    <div class=" container">
        <div class="row align-items-end">
            <div class=" col-lg-6 col-12">
                <div class=" common-heading">
                    <span class="sub-heading" id="{{ ($section->product_category->section->sub_title->slug ?? '') }}_preview"> {!! $section->product_category->section->sub_title->text ?? "" !!}</span>
                    <h2  id="{{ ($section->product_category->section->title->slug ?? '') }}_preview">{!! $section->product_category->section->title->text ?? "" !!} </h2>
                    <p  id="{{ ($section->product_category->section->description->slug ?? '') }}_preview">{!! $section->product_category->section->description->text  !!}</p>
                    <div class="d-flex justify-content-left align-items-center">
                        <a href="{{ route('page.product-list', $slug) }}" class="common-btn">
                            <span  id="{{ ($section->product_category->section->button->slug ?? '') }}_preview">{!! $section->product_category->section->button->text !!}</span>
            ...
            <div class=" col-lg-6 col-12">
                <div class="fifth-sec-right-slider">
                    @foreach ($home_products as $data)
                        <div class="main-card product-card card col-lg-3 col-sm-6 col-12">
                            <div class="pro-card-inner">
                                ...
                                <div class="inner-card">
                                    ...
                                    @endif
                                    <a href="JavaScript:void(0)" class="common-btn addtocart-btn addcart-btn-globaly"
